...
class UserCommentServiceTest {
  ...
  @Mock UserCommentDTOUserCommentMapper userCommentDTOUserCommentMapper;

  @InjectMocks UserCommentService userCommentService;
  ...
  void setUp() {
    ...
    loggedInAuthor = new UserTestBuilder().fromUser(author).buildUserPrincipal();
    ...
    loggedInNotAuthor = new UserTestBuilder().fromUser(notAuthor).buildUserPrincipal();
    ...
    userCommentDTO1 =
        new UserCommentTestBuilder().fromUserComment(userComment1).buildUserCommentDTO();

    userCommentDTO2 =
        new UserCommentTestBuilder().fromUserComment(userComment2).buildUserCommentDTO();
  ...
  void getUserCommentDTOById_returnsComment() {
    when(userCommentRepository.findById(userComment1.getId()))
  ...
  void getUserCommentDTOById_throwsExceptionIfNotFound() {
    ...
    assertThrows(
        ...
        () -> userCommentService.getUserCommentDTOById(userComment1.getId()));
  ...
  void getUserCommentById_throwsExceptionIfNotFound() {
    ...
    assertThrows(
        UserCommentNotFoundException.class,
        () -> userCommentService.getUserCommentById(userComment1.getId()));
  ...
  void getUserCommentsByAuthor_returnsComments() {
    when(userCommentRepository.findUserCommentsByAuthor(author)).thenReturn(userComments);
    when(userCommentDTOUserCommentMapper.mapUserCommentsToUserCommentDTOs(userComments))
        .thenReturn(userCommentDTOs);
  ...
  void getUserCommentsByAuthor_returnsEmptyListIfNoComments() {
    when(userCommentRepository.findUserCommentsByAuthor(author)).thenReturn(userCommentsEmpty);
    when(userCommentDTOUserCommentMapper.mapUserCommentsToUserCommentDTOs(userCommentsEmpty))
        .thenReturn(userCommentDTOsEmpty);
  ...
  void getUserCommentsByEvent_returnsComments() {
    when(userCommentRepository.findUserCommentsByEvent(event)).thenReturn(userComments);
    when(userCommentDTOUserCommentMapper.mapUserCommentsToUserCommentDTOs(userComments))
        .thenReturn(userCommentDTOs);
  ...
  void getUserCommentsByEvent_returnsEmptyListIfNoComments() {
    when(userCommentRepository.findUserCommentsByEvent(event)).thenReturn(userCommentsEmpty);
    when(userCommentDTOUserCommentMapper.mapUserCommentsToUserCommentDTOs(userCommentsEmpty))
        .thenReturn(userCommentDTOsEmpty);
  ...
  void getUserCommentByEventIdAndCommentId_returnsComment() {
    ...
    UserCommentDTO result =
        userCommentService.getUserCommentByEventIdAndCommentId(event.getId(), userComment1.getId());
  ...
  void getUserCommentByEventIdAndCommentId_throwsExceptionIfNotFound() {
    ...
    assertThrows(
        UserCommentNotFoundException.class,
        () ->
            userCommentService.getUserCommentByEventIdAndCommentId(
                event.getId(), userComment2.getId()));

    verify(userCommentRepository)
        .findUserCommentByEventIdAndId(event.getId(), userComment2.getId());
  }
  ...
  void addNewUserCommentToEventById_addsNewComment() {
    ...
    when(userCommentDTOUserCommentMapper.mapUserCommentDTOToUserComment(
            userCommentDTO2, author, event))
        .thenReturn(userComment2);
    when(userCommentRepository.save(userComment2)).thenReturn(userComment2);
    when(userCommentDTOUserCommentMapper.mapUserCommentToUserCommentDTO(userComment2))
        .thenReturn(userCommentDTO2);

    UserCommentDTO result =
        userCommentService.addNewUserCommentToEventById(event.getId(), userCommentDTO2);
    ...
    verify(userCommentDTOUserCommentMapper)
        .mapUserCommentDTOToUserComment(userCommentDTO2, author, event);
  ...
  void addNewUserCommentToEventById_throwsExceptionIfEventNotFound() {
    ...
    assertThrows(
        ...
        () -> userCommentService.addNewUserCommentToEventById(event.getId(), userCommentDTO1));
  ...
  void updateUserCommentByEventIdAndCommentId_updatesComment() {
    ...
    UserCommentDTO result =
        userCommentService.updateUserCommentByEventIdAndCommentId(
            event.getId(), userComment1.getId(), userCommentDTO1, loggedInAuthor);
  ...
  void updateUserCommentByEventIdAndCommentId_throwsExceptionIfUserIsNotAuthor() {
    ...
    doThrow(new UserNotAuthorizedException("You are not allowed to update this comment"))
        ...
        .assertCanUpdateComment(userComment1, notAuthor);

    assertThrows(
        UserNotAuthorizedException.class,
        () ->
            userCommentService.updateUserCommentByEventIdAndCommentId(
                event.getId(), userComment1.getId(), userCommentDTO1, loggedInNotAuthor));
  ...
  void deleteUserCommentByEventIdAndCommentId_deletesComment() {
    ...
    UserCommentDTO result =
        userCommentService.deleteUserCommentByEventIdAndCommentId(
            event.getId(), userComment1.getId(), loggedInAuthor);
  ...
  void deleteUserCommentByEventIdAndCommentId_throwsExceptionIfUserIsNotAuthor() {
    ...
    doThrow(new UserNotAuthorizedException("You are not allowed to delete this comment"))
        .when(accessControlService)
        .assertCanDeleteComment(userComment1, notAuthor);

    assertThrows(
        UserNotAuthorizedException.class,
        () ->
            userCommentService.deleteUserCommentByEventIdAndCommentId(
                event.getId(), userComment1.getId(), loggedInNotAuthor));
